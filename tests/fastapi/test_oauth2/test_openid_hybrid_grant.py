...
from .models import (Client, CodeGrantMixin, User, db, exists_nonce,
...
class OpenIDCodeTest(TestCase):
    ...
    def test_require_nonce(self):
        ...
        self.assertIn("error=invalid_request", rv.headers["location"])
        self.assertIn("nonce", rv.headers["location"])
    ...
    def test_code_access_token(self):
        ...
        self.assertNotIn("id_token=", rv.headers["location"])

        params = dict(url_decode(urlparse.urlparse(rv.headers["location"]).fragment))
    ...
    def test_response_mode_form_post(self):
        ...
        resp = rv.json()
        self.assertIn('name="code"', resp)
        self.assertIn('name="id_token"', resp)
        self.assertIn('name="access_token"', resp)
