...
from quart import request as quart_req
from quart import _app_ctx_stack
from ..base_client import RemoteApp


class QuartRemoteApp(RemoteApp):
    """Quart integrated RemoteApp of :class:`~authlib.client.OAuthClient`.
    ...
    def __init__(self, framework, name=None, fetch_token=None, **kwargs):
        ...
        super(QuartRemoteApp, self).__init__(framework, name, fetch_token, **kwargs)
    ...
    def _on_update_token(self, token, refresh_token=None, access_token=None):
        self.token = token
        super(QuartRemoteApp, self)._on_update_token(
    ...
    async def request(self, method, url, token=None, **kwargs):
        if token is None and not kwargs.get('withhold_token'):
            token = self.token
        return super(QuartRemoteApp, self).request(
    ...
    async def authorize_redirect(self, redirect_uri=None, **kwargs):
        ...
        self.save_authorize_data(quart_req, redirect_uri=redirect_uri, **rv)
    ...
    async def authorize_access_token(self, **kwargs):
        ...
        params = self.retrieve_access_token_params(quart_req, request_token)
    ...
    def parse_id_token(self, token, claims_options=None, leeway=120):
        return self._parse_id_token(quart_req, token, claims_options, leeway)
