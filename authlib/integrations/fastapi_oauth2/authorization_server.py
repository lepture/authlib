...
class AuthorizationServer(_AuthorizationServer):
    ...
    def init_app(self, app, query_client=None, save_token=None):
        ...
        if metadata_file:
            with open(metadata_file) as f:
                metadata = self.metadata_class(json.loads(f))
                metadata.validate()
    ...
    def create_oauth2_request(self, request):
        return OAuth2Request(request.method, str(request.url), request.body, request.headers)

    def create_json_request(self, request):
        return HttpRequest(request.method, str(request.url), request.body, request.headers)
