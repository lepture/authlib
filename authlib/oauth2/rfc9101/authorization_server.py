...
class JWTAuthenticationRequest:
    ...
    def parse_authorization_request(
        ...
        client = self._get_client(authorization_server, request)
    ...
    def _shoud_proceed_with_request_object(
        ...
        if self.get_client_require_signed_request_object(client):
            raise InvalidRequestError(
                "Authorization requests for this client must use request objects.",
                state=request.payload.state,
            )
        ...
        if metadata and metadata.get("require_signed_request_object", False):
            raise InvalidRequestError(
                "Authorization requests for this server must use request objects.",
                state=request.payload.state,
            )
    ...
    def _get_client(
        ...
        if request.payload.client_id is None:
            raise InvalidClientError(
                state=request.payload.state,
                description="Missing 'client_id' parameter.",
            )

        client = authorization_server.query_client(request.payload.client_id)
        if not client:
            raise InvalidClientError(
                state=request.payload.state,
